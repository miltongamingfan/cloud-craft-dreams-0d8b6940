...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = { Box, Cloud, Monitor, Server };

export function CategoryPlans({ slug }: { slug: string }) {
  ...
  if (!category) {
    return <div className="mx-auto max-w-5xl px-6 py-24 text-center text-muted-foreground">Loading…</div>;
  }
